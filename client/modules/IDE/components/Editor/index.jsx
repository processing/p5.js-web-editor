...
function Editor(props) {
  ...
  const funcprop = bindActionCreators(
    Object.assign(
      {},
  ...
  const updateLintingMessageAccessibility = debounce((annotations) => {
    ...
    if (mergedState.lintMessages.length > 0 && mergedState.lintWarning) {
      beep.play();
    }
  }, 2000);
  useEffect(() => {
  ...
  }, [mergedState.lintMessages.length, mergedState.lintWarning]);
  ...
  }, [file.id, mergedState.autorefresh, mergedState.isPlaying]);

  // function for comparing previous states values
  const updatedom = () => {
    if (mergedState.fontSize !== prevProps.current.fontSize) {
      cm.current.getWrapperElement().style[
        'font-size'
      ] = `${mergedState.fontSize}px`;
    }
    if (mergedState.linewrap !== prevProps.current.linewrap) {
      cm.current.setOption('lineWrapping', mergedState.linewrap);
    }
    if (mergedState.theme !== prevProps.current.theme) {
      cm.current.setOption('theme', `p5-${mergedState.theme}`);
    }
    if (mergedState.lineNumbers !== prevProps.current.lineNumbers) {
      cm.current.setOption('lineNumbers', mergedState.lineNumbers);
    }
    if (
      mergedState.autocloseBracketsQuotes !==
      prevProps.current.autocloseBracketsQuotes
    ) {
      cm.current.setOption(
        'autoCloseBrackets',
        mergedState.autocloseBracketsQuotes
    ...
    if (
      mergedState.autocompleteHinter !== prevProps.current.autocompleteHinter
    ) {
      if (!mergedState.autocompleteHinter) {
        // close the hinter window once the preference is turned off
        CodeMirror.showHint(cm.current, () => {}, {});
      }
    }

    if (mergedState.runtimeErrorWarningVisible) {
      if (consoleEvents && prevProps.current.consoleEvents) {
        if (consoleEvents.length !== prevProps.current.consoleEvents.length) {
          consoleEvents.forEach((consoleEvent) => {
            if (consoleEvent.method === 'error') {
              ...
              StackTrace.fromError(errorObj).then((stackLines) => {
                ...
                const fileWithError = mergedState.files.find(
    ...
    if (file.id !== prevProps.current.fileid) {
      for (let i = 0; i < cm.current.lineCount(); i += 1) {
        cm.current.removeLineClass(i, 'background', 'line-runtime-error');
      }
    }
  ...
  // useEfectWithComparison(updatedom, props);
  // useEffect of future
  useEffect(() => {
    if (file.id !== prevProps.current.fileid) {
      const oldDoc = {};
      ...
      cm.current.focus();
      if (!prevProps.current.unsavedChanges) {
